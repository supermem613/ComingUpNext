...
namespace ComingUpNextTray.Services {
    public sealed class CalendarService {
        ...
        public async Task<IReadOnlyList<CalendarEntry>> FetchAsync(string calendarIcsUrl, CancellationToken ct = default) {
            ...
            try {
                using HttpResponseMessage resp = await _httpClient.GetAsync(calendarIcsUrl, ct);
        ...
        internal static IReadOnlyList<CalendarEntry> ParseIcs(string ics) {
            ...
            using (StringReader reader = new StringReader(ics)) {
                string? line;
                string? prev = null;
                    ...
                    switch (key) {
                        ...
                        case "DESCRIPTION":
                            if (string.IsNullOrWhiteSpace(current.MeetingUrl)) {
                                ...
                                if (idx >= 0) {
                                    string? segment = value.Substring(idx).Split('\n', ' ', '\r', '\t').FirstOrDefault();
        ...
        internal static DateTime ParseDate(string val) {
            ...
            try {
                if (val.EndsWith("Z", StringComparison.OrdinalIgnoreCase)) {
                    ...
                    if (DateTime.TryParseExact(core, "yyyyMMdd'T'HHmmss", null, System.Globalization.DateTimeStyles.AssumeUniversal | System.Globalization.DateTimeStyles.AdjustToUniversal, out DateTime utc)) {
                        return utc.ToLocalTime();
                    }
                }
                if (val.Length == 8 && DateTime.TryParseExact(val, "yyyyMMdd", null, System.Globalization.DateTimeStyles.None, out DateTime dateOnly)) {
                    // treat all-day as local midnight
                    return dateOnly;
                }
                if (DateTime.TryParseExact(val, "yyyyMMdd'T'HHmmss", null, System.Globalization.DateTimeStyles.AssumeLocal, out DateTime local)) {
                    return local;
                }
            }
            catch { }
